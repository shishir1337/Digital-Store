...
import { heroImages, exclusiveOffers } from "@/lib/constants"
import { hexToRgba } from "@/lib/utils/hex-to-rgba"
import type { HeroImage, ExclusiveOffer } from "@/types/hero"
...
export function Hero() {
  ...
  const prevIndex = (current - 1 + heroImages.length) % heroImages.length
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(to bottom, ${hexToRgba(heroImages[current].color, 0.70)}, ${hexToRgba(heroImages[current].color, 0.20)}, ${hexToRgba(heroImages[current].color, 0.05)}, var(--gradient-end))`,
        }}
      />
      ...
      <div className="container mx-auto max-w-[1440px] px-2 sm:px-4 relative z-10">
        ...
        >
          <div className="relative w-full aspect-[2.5/1] sm:aspect-[2.8/1] rounded-lg sm:rounded-xl overflow-hidden shadow-xl ring-1 ring-black/5 dark:ring-white/10 group">
            <Link href={heroImages[current].link} className="block w-full h-full">
              <Image
        ...
        >
          ...
          >
            <Image
              src={heroImages[prevIndex].src || "/placeholder.svg"}
          ...
          <div
            ...
            aria-label={`Slide ${current + 1} of ${heroImages.length}: ${heroImages[current].alt}`}
          >
            <Link href={heroImages[current].link} className="block w-full h-full">
              <Image
                src={heroImages[current].src || "/placeholder.svg"}
          ...
          >
            <Image
              src={heroImages[nextIndex].src || "/placeholder.svg"}
